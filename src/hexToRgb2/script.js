...
(() => {
  ...
  */
	...
	const handleRgbInput = () => {
		const vals = inputs.map(value => Number(value.value));
	...
	const handleHexInput = (event_ = {}) => {
        ...
        */
		...
		if (typeof origValue === 'string' && validLengths.has(origValue.length)) {
			...
			let parsedValue = Number.parseInt(properLengthValue, 16);
	...
	const handleScroll = event_ => {
		if (event_.target.nodeName === 'INPUT') {
			const newValue = Number(event_.target.value) - Math.sign(event_.deltaY);
			/* Negative because it is more intuitive
       where scrolling up increases,
       scrolling down decreases */

			event_.target.value = newValue > 255 ? 255 : (newValue < 0 ? 0 : newValue); // Limit to [0, 255]
	...
	document.querySelector('#hex').addEventListener('input', handleHexInput);

	const rgbInputs = document.querySelector('#rgb');

	rgbInputs.addEventListener('input', handleRgbInput);

	rgbInputs.addEventListener('wheel', handleScroll, {passive: true});
	document.querySelector('#rand').addEventListener('click', randomise);
